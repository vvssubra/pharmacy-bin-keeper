...
Deno.serve(async (req) => {
  ...
  const { systemPrompt, dataForClaude } = buildSystemPrompt(role, dataContext);
...
function buildSystemPrompt(role: string, dataContext: object): { systemPrompt: string; dataForClaude: string } {
  if (role === "mo") {
    ...
    const systemPrompt = `You are a clinical and pharmacy assistant for a Malaysian government clinic.
${clinicalBlock}
...
${dataStr}${nagSection}`;
    ...
    return { systemPrompt, dataForClaude: dataStr };
  }

  const dataStr = JSON.stringify(dataContext);
  const systemPrompt = `You are a pharmacy management assistant. Answer ONLY from the data provided below.
